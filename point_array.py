""" Operations that take place on arrays of points """
from point import add_points, invert_point, scale_point, squared_dist_points
...
def squared_difference(point_array_1, point_array_2):
    """ Sums the squared difference between two point arrays. Used to
    calculate the average error of a filtering method """
    assert len(point_array_1) == len(point_array_2)
    squared_delta = 0
    for p_id, point_1 in enumerate(point_array_1):
        point_2 = point_array_2[p_id]
        squared_delta += squared_dist_points(point_1, point_2)
